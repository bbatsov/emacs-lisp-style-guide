...
** Source Code Layout & Organization
   ...
   - Avoid parameter lists with more than three or four positional
     ...
     large extent, as you constantly see the signature of a function in
     the minibuffer.
   ...
   - Always enable [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Lexical-Binding.html][lexical scoping]]. Lexical scoping reduces the risk of
     unintended variable capture, makes closures work as expected, and
     enables additional byte-compiler optimizations. There's rarely a
     reason to use dynamic scoping for new code. This must be done on the
     first line as a file local variable.
...
** Syntax
   ...
   - Use =not= instead of =null=, unless you're checking whether something is
...
** Macros
   ...
   - Don't write a macro if a function will do. Macros don't compose
     like functions -- you can't =funcall=, =apply=, or pass them as
     arguments -- so they should only be used when you actually need
